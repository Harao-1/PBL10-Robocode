...
public class MovingRobot extends AntiGrav{
	...
	public MovingRobot(MessagePack thePack){
		ScannedRobotEvent e = thePack.getEvent();
		double scannedRobotRadians = thePack.getHeading() + e.getBearingRadians();

		x = thePack.getX() + e.getDistance() * Math.sin(scannedRobotRadians);
		y = thePack.getY() + e.getDistance() * Math.cos(scannedRobotRadians);
		velocityX = e.getVelocity() * Math.sin(e.getHeadingRadians());
		velocityY = e.getVelocity() * Math.cos(e.getHeadingRadians());
